...
class SECrawler(AbstractBaseCrawler):
	...
	def is_article(self, url):
		if self.sde_level_articles(url) and self.partial_links_to_ignore(url) and not \
		Article.objects.filter(article_url=url).exists() and '/article/' in url:
			return True
		else:
			return False
			...
			if get_categories.status_code == 200:
				...
				for category in all_categories:
					if (category.get('href') is not None):
						...
						self.partial_links_to_ignore(cat) and cat.startswith('https://www.standardmedia.co.ke/entertainment/category/'):
	...
	def update_article_details(self, link):
		...
		if story.status_code == 200:
			...
			article_json_data =  [json.loads(x.string) for x in soup.find_all("script", type="application/ld+json")][0]

			date_string = article_json_data['datePublished']		
			publication_date = self.create_datetime_object_from_string(date_string)
			date = pytz.timezone("Africa/Nairobi").localize(publication_date, is_dst=None)

			author = [article_json_data['author']['name']]
			try:
